...
public class ReportingController {
    ...
    public ResponseEntity<Map<String, Object>> getDashboardStats() {
        ...
        // ⚡ Bolt: Replace in-memory list fetch & size() with DB-level count
        long opdCount = queueRepository.countByTokenDate(LocalDate.now());

        // ⚡ Bolt: Replace in-memory findAll() & stream filtering with DB-level count
        ...
        // ⚡ Bolt: Replace in-memory fetch with DB-level count
        long pendingLabsCount = labRequestRepository.countByStatus(LabRequest.RequestStatus.PENDING);

        // ⚡ Bolt: Replace in-memory list fetch & stream mapToDouble().sum() with DB-level SUM query
